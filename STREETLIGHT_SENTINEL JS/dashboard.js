(function() {
    ...
    window.initMap = function() {
        ...
        window.groups.forEach(g => {
            ...
            let fault = g.subunits.some(s => s.fault);
    ...
    window.updateStats = function() {
        ...
        document.getElementById('groupsOnline').innerHTML = `${connected.length} <span class="stat-unit">/5</span>`;
        let faults = connected.reduce((a, g) => a + g.subunits.filter(s => s.fault).length, 0);
